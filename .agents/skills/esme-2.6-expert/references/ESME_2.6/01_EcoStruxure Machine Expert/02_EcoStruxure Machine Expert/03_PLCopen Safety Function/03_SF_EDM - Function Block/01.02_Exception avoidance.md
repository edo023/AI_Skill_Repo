...
* At the edge-triggered Reset input, such signals cause the function block to interpret the signal as an edge and trigger a potentially undesirable corresponding action.
...
* Contact block being removed from the contactor (user errors)
* Contacts sticking (hardware errors).
...
If a start-up inhibit is specified by the setting S\_StartReset = SAFEFALSE, a static TRUE signal at the Reset input when the Safety Logic Controller starts up leads to a function block error message (Error = TRUE).

If the start-up inhibit is not in use when the Safety Logic Controller starts up (S\_StartReset = SAFETRUE), the signal state at the Reset input is not relevant at this time. In such cases, the signal at the S\_EDM\_Out output depends exclusively

* on the state of the control signal at the S\_OutControl input
* **and** on the states of the feedback signals at inputs S\_EDM1 and S\_EDM2.
