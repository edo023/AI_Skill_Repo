...
If the start-up inhibit is not in use when the Safety Logic Controller starts up (S\_StartReset = SAFETRUE), the signal state at the Reset input is not relevant at this time. In such cases, the signal at the S\_ESPE\_Out output depends exclusively

* on the status of the connected safety equipment
* **and** on the setting at S\_AutoReset (restart inhibit).

## Impermissible static TRUE at Reset input

If the function block is expecting a positive edge for resetting (start-up inhibit is active or an error state applies), a static TRUE signal at Reset will be regarded as an error. In such cases, switch Reset to FALSE to exit the error state.
