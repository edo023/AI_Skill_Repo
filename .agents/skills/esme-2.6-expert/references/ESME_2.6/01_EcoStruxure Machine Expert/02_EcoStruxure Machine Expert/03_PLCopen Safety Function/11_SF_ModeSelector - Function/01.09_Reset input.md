...
Edge-triggered input for resetting error messages if the cause of the error is no longer present.
...
Resetting the function block by means of a positive signal edge at the Reset input can cause the S\_Mode0Sel..S\_Mode7Sel output to switch to SAFETRUE immediately (depending on the status of the other inputs).
