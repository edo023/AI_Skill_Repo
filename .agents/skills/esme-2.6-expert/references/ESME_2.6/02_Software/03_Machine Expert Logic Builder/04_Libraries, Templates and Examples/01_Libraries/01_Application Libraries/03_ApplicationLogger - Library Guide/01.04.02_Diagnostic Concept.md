...
EcoStruxure Machine Expert provides a three-layer diagnostic concept for the libraries. This concept is valid for the Technology/Module libraries (for example, the library PD\_PacDrive.lib) and uses enumerations for diagnostic coding.
...
The diagnostic output q\_etDiag of the type GD.ET\_Diag provides library-independent diagnostic information, for example `InputParameterInvalid`. The information can suggest a solution for the cause of the diagnostic.

Depending on the value of GD.ET\_Diag, it is either a status description or an exception message. A value not equal to GD.ET\_Diag.Ok indicates an exception message.
...
Alternatively, the results of the function can be returned using several outputs; in this case the direct return value of the function is a BOOL value with a random value that cannot be interpreted.
...
An exception exists if the return value is <> GD.ET\_Diag.Ok. The functions are processed completely in the call-up task, then their status can be evaluated and they report using q\_etDiag and q\_etDiagExt whether their processing was successful.
