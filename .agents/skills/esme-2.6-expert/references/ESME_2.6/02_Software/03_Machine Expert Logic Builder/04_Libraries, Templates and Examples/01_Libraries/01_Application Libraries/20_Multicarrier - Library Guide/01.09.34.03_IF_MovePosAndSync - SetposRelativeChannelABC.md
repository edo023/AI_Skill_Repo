# IF\_MovePosAndSync - SetposRelativeChannelABC (Method)
...
NOTE: In a Lexium™ MC multi carrier track, you can use a combination of move commands like MovePosAndSync and MoveGapControl for different carriers at the same time. Keep in mind that the MovePosAndSync commands on channel B and C for the selected carrier are not taken into account by the carrier in front or behind that uses, for example, the move command [MoveGapControl](IF_MoveGapControl-5B81ACFA.html).

With the move command MovePosAndSync.SetposRelativeChannelABC, the carrier uses the positioning or the cam commands of the SoMotionGenerator without considering the other carriers. Take this into account during path planning.
...
Ensure that no move command is active for the carrier before calling the method IF\_MovePosAndSync - SetposRelativeChannelABC.
...
```
...ifMovePosAndSync.SetposRelativeChannelABC(…)
```
