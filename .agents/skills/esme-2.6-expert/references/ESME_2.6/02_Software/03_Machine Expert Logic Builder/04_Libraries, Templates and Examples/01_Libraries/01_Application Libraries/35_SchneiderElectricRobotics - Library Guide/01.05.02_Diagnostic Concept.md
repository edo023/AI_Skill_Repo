...
* Detailed precision of diagnostic events with the availability of the diagnostic codes
...
Depending on the value of GD.ET\_Diag, it is either a status description or an exception message. A value not equal GD.ET\_Diag.Ok equates to an exception message.
...
Read the value q\_etDiag to determine if the function was successful.

An exception exists if the q\_etDiag is <> GD.ET\_Diag.Ok. The functions are processed completely in the call-up task, then their status can be evaluated and they report with q\_etDiag and q\_etDiagExt whether their processing was successful.
