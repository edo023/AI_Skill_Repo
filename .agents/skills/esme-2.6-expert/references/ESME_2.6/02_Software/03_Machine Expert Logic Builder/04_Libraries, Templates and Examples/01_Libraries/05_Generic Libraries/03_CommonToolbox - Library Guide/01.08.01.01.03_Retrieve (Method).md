...
Reads and removes the earliest value stored in the stack.

## Description

The method Retrieve returns the first value stored inside the stack. When a value is retrieved it is removed from the stack.
...
| q\_xError = TRUE  q\_etResult = RegisterEmpty | No element can be retrieved as stack is empty. | Store first a value to the stack before retrieving one. |
