# IF\_MovePosAndSync - StartSyncStep (Method)
...
![](images/DD00900595.png)

## Task

Execution of a step movement of the carrier synchronized with a master.

## Description

The method StartSyncStep allows the execution of a step movement of a carrier synchronized with a master. This means that the connected carrier is moved to a new target position while synchronizing with the master. After phasing, the job is finished.
...
As a precondition for calling the method StartSyncStep, the selected channel of the carrier must be in standstill. The master can be in motion and on the further channels, other move commands can be active. The value of the parameter RefVelocity for the selected channel must be 0.
...
Before executing the method StartSyncStep, the method SetMotionParameter must be called at least once because the motion parameters are needed when calling a stop method.

Example:

```
...ifMotion.SetMotionParameter(...)
...ifMovePosAndSync.StartSyncStep(...)
```
