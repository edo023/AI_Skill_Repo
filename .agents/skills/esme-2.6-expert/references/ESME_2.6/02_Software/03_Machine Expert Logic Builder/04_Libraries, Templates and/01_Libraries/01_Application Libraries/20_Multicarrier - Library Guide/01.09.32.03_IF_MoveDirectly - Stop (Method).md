...
![](images/0000758618.png)

## Task

Stopping the carrier movement controlled by the move command [MoveDirectly](IF_MoveDirectly-5B82D7F8.html#IF_MoveDirectly-5B82D7F8).

## Description

The method IF\_MoveDirectly - Stop stops a carrier movement started by the method [IF\_MoveDirectly - Start](IF_MoveDirectly-StartMethod-5445716E.html#IF_MoveDirectly-StartMethod-5445716E).

When the method IF\_MoveDirectly - Stop is called, the carrier is stopped with a positioning command setting the velocity to zero:
...
With the method IF\_MoveDirectly - Stop, the movement of the carrier is stopped without considering other carriers, for example without considering if the carrier in front stops faster. Take this into account during path planning.
