...
This document describes the functionalities contained in the PD\_MultiBelt library.
...
| WARNING | |
| --- | --- |
|  | UNINTENDED MOVEMENT OF THE Subordinate AXIS  Deactivate the POU that instructs the subordinate, or disconnect the connection from the master, if the subordinate axis stops independently from the master.  Failure to follow these instructions can result in death, serious injury, or equipment damage. |
