...
| SingleShot | BOOL | Value range: FALSE, TRUE.  Default value: TRUE.  Selects the trigger occurrence.   * FALSE = Captures continuously * TRUE = Captures once |
...
| TriggerEdge | ET\_TriggerEdge | Edge to trigger position capture.   * 1 / RisingEdge: Rising edge. * 2 / FallingEdge: Falling edge. * 3 / BothEdges: Both rising edge and falling edge(1).   See also [Vendor-Specific Data Type ET\_TriggerEdge](D-SE-0093799.html#D-SE-0093799__D-SE-0093799.8). |
| **(1)** Only with EtherNet/IP and Modbus TCP. | | | | |

## Notes

Use MC\_AbortTrigger function block to abort TouchProbe\_ILX function block execution.
