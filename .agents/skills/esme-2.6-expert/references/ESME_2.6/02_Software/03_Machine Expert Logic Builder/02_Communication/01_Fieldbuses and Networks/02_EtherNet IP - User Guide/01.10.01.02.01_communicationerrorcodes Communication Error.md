...
| --- | --- | --- |
| `CommunicationOK` | `00` | The exchange is valid. For more details on the transaction status, refer to [OperationErrorCodes: Operation Error Codes](D-SE-0058093.html). |
...
| `ProtocolSpecificError` | `FE` | The operation error code contains a protocol-specific code.  The message was sent but was not received by the target, or the target has replied with an error code. For more details on the protocol-specific code status, refer to [OperationErrorCodes: Operation Error Codes](D-SE-0058093.html). |
| `Refused` | `FF` | The message was refused. For more details on the message refused status, refer to [OperationErrorCodes: Operation Error Codes](D-SE-0058093.html). |
