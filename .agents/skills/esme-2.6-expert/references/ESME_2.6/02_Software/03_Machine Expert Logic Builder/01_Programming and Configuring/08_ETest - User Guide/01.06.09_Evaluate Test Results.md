...
The ETEST framework provides the test results of a completed test and displays the progress of a running test in the Test results view.
...
| Buttons | Description |
| --- | --- |
| Export | Stores selected test results in the following XML formats:   * *\*.testresult* * *\*.xml* (JUnit-compatible) |
...
Imported test results are also opened in the area Test results and displayed in the list of the most recently executed tests. The area Test results is automatically opened when a test is started. It displays the result (or the state) of a test.
...
| Tab | Description |
| --- | --- |
| Test results | Tree view which displays the test series and test cases.  It displays the individual results for each node. |
| User comment | User name and comment.  Displays by default the Windows user name.  You can add a comment to the Comment box. This data is stored together with the test result. |
...
A selected part of the found string (in this example, Test) is marked yellow. The list displays the marked entries and its parent nodes.
