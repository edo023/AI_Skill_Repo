...
| 3 | Select a Counting function that supports the Enable function:   * HSC Main Single Phase (One-shot or Modulo-loop) * HSC Main Dual Phase (Modulo-loop or Free-large) * Frequency Meter * Period Meter |
| 4 | Set the value of the Control inputs > EN input > Location parameter. |
| 5 | Select the value of the Control inputs > EN input > Filter parameter to reduce the bounce effect on the input.  The filtering value determines the counter maximum frequency as shown in the [Bounce Filter table](D-SE-0003769.html#D-SE-0003769__D-SE-0003769.3). |
