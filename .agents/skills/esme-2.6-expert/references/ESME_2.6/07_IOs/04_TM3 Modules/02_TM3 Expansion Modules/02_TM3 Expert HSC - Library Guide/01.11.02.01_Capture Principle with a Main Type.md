...
* use the `CapturedValue` parameter of the [HSCMain\_TM3](D-SE-0007013.html#D-SE-0007013) function block to retrieve the captured value in your application.

## Principle of a Capture

This graphic illustrates how the capture works in Modulo-loop mode:

![](images/T001662952.gif)
...
| 3 | Get the stored value using the `CapturedValue` parameter of the [HSCMain\_TM3](D-SE-0007013.html#D-SE-0007013) function block. |
| 4 | While `Cap_Flag` = 1, any new edge on the physical input CAP is ignored. |
| 5 | The rising edge of `HSCMain_TM3` function block input `ACK_Cap` triggers the falling edge `Cap_Flag` output.  A new capture is authorized. |
