...
| 3 | If data are available, copy the block of serial data starting from RxBytex. Save the current write pointer position and add the new frame length.  Calculate the residual length of the segment. The next RxBytex may already be a control byte for the next segment or frame. When frame end detection is set and the data have been copied, mark the frame as complete. |
