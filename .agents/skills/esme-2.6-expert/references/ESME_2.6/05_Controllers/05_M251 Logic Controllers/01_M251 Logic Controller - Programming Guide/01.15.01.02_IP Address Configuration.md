...
In the Devices tree, double-click Ethernet\_1 or Ethernet\_2 (only available on TM251MESE):
...
* For Ethernet\_2, IP Forwarding protocol is not available and DHCP Server active is selected by default.
...
* 10.11.x.x. for Ethernet\_1
* 10.10.x.x. for Ethernet\_2 (only available on TM251MESE)
...
The gateway address can be defined on Ethernet\_1 interface. The traffic to unknown networks is sent through this gateway address, or to address configured on [IP routing table](EthernetServices-A6347034.html#EthernetServices-A6347034__IPRouting-M251).
...
| IP Forwarding | This parameter activates/deactivates the IP Forwarding service of the controller. When deactivated, devices on the device network are no longer accessible from the control network (Web pages, DTM, and so on).  NOTE: This parameter is only available on the Ethernet\_1 network. | Inactive |
...
When DHCP Server active is selected, devices added to the fieldbus can be configured to be identified by their name or MAC address, instead of their IP address. Refer to [DHCP Server](D-SE-0037734.html#D-SE-0037734).

NOTE: This parameter is only available on the Ethernet\_2 network.
