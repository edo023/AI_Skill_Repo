...
| 3 | Function Blocks are called using a `CAL` instruction:   * Use the Input Assistant to select the FB (right-click and select Insert Box in the context menu). * Automatically, the `CAL` instruction and the necessary I/O are created.   Each parameter (I/O) is an instruction:   * Values to inputs are set by "`:=`". * Values to outputs are set by "`=>`". |
