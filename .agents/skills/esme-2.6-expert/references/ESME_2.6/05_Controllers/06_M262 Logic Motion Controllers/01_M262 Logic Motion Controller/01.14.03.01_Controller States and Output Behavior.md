...
NOTE: For TM3 **Expert module** reflex output behavior, refer to [Modicon TM3 Expansion Modules Configuration - Programming Guide](../../../../../api/crossBook?lang=en-US&virtualBookName=tm3ioprg&topicID=D_SE_0094027).
...
This output behavior applies in the STOPPED controller state. It also applies to [CAN bus in the HALT controller state](D-SE-0008844.html#D-SE-0008844__CANopenBehaviorInHALTState-3B616DAB), except if the task defined for the Bus cycle task of the CANopen\_Performance is in Exception, or during ProcessorLoadWatchdog.

Outputs maintain their state, although the details of the output behavior vary greatly depending on the setting of the Update I/O while in stop option and the actions commanded via configured fieldbuses. Refer to [PLC Settings](D-SE-0006801.html) for more details on these variations.
