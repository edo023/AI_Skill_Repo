...
If the start-up inhibit is not in use when the Safety Logic Controller starts up (S\_StartReset = SAFETRUE), the signal state at the Reset input is not relevant at this time. In such cases, the signal at the S\_EStopOut output depends exclusively
