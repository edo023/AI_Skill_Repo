...
The following conditions lead to an error message being produced at the function block (Error = TRUE and S\_OSSD\_Out = SAFEFALSE at the same time).

* Exceeding the test time set at TestTime during the sensor test.
* No response from the sensor during the sensor test.
* Invalid static TRUE signals at Reset.
* Invalid value for TestTime.
...
If the start-up inhibit is not in use when the Safety Logic Controller starts up (S\_StartReset = SAFETRUE), the signal state at the Reset input is not relevant at this time. In such cases, the signal at the S\_OSSD\_Out output depends exclusively
