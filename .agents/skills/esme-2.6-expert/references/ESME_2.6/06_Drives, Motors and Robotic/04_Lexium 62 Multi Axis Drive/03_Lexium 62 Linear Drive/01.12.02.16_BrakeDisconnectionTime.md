...
With this parameter you can set the brake disconnection time. The disconnection time starts, when the brake is switched over electrically, so that its status changes from engaged (coupled) to released. After the disconnection time, the drive accepts travel tasks. Usually this procedure is triggered because the drive goes in closed-loop control after switching on ControllerEnable. The parameter [AxisState](../State_2/State_2-15.htm#XREF_D_SE_0071486_1) switches from 1 to 2 and the parameter [BrakeState](Motor_2-13.htm#XREF_D_SE_0071790_1) switches from 0 / coupled to 1 / released. After the disconnection time, the AxisState switches to 3.
