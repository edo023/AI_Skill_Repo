...
The parameter is used to indicate the force constant of the motor [N/A] (read from the motor encoder) for low currents. For currents which are greater than the nominal current, the torque constant reduces due to saturation effects in the motor. This effect is taken into account when calculating the maximum acceleration MaxAcc.
