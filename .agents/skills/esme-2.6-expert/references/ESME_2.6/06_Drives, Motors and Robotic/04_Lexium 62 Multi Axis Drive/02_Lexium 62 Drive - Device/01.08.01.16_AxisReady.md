...
To simplify querying the axis job status in the program, the AxisReady parameter was implemented. It is derived from the parameter AxisState.

| Value | Data type | Meaning |
| --- | --- | --- |
| no / 0 | BOOL | Currently, no process tasks can be processed (error detected or stop has occurred).  AxisState is 0, 1, 2, 7 or 8. |
| yes / 0 | BOOL | Process tasks can be processed.  AxisState is 3, 4 or 5. |
